...
> 这是 issh 视觉与交互设计的**执行手册**。对照 paseo design.md 的章节结构，
> 内容是 issh 自己的设计语言 — M22-M37 累积下来的具体做法 + forbidden 清单。
>
...
issh-ui 把设计 token 分 6 层（按抽象高度倒序）：
...
**issh 用 typography 9 个 role 表达层级，不靠手写 font_size / font_weight。**
...
issh 的按钮 primitive 是 `issh_ui::Button`，**4 个 variant**（不是 5 个）：
...
**注意**：issh **没有** Outline variant —— Ghost + 在 row 末尾的视觉位置已经覆盖
...
issh 用 `anatomy.rs` 控密度，三级 rhythm：
...
issh **桌面 only**（macOS / Windows / Linux），不做 mobile / web。但 GUI 内有
两个 form factor：
...
issh 是中文 UI，规则跟 paseo 英文 sentence case 不同，但「一致 + 简短」原则一致：
...
issh 的状态呈现已经成体系（M28 state design），4 类：

| 状态 | 用 | 文件 |
|---|---|---|
| **Page-level 通知** | `toast_info` / `toast_warning` / `toast_error` | `issh_ui::toast` |
...
issh 已有 `Badge` (`components/badge.rs`)，5 个 variant：
...
## 13. Forbidden / 不要这么干（issh 自己踩过的坑）
...
| 模式 | 参考实现 |
|---|---|
| Home Launchpad（active 卡 + saved 卡） | `issh-app/src/views/home.rs` (M36) |
| Settings 页（720 居中 row list） | `issh-app/src/views/settings.rs` |
| Host 表单 Dialog | `issh-app/src/views/host_form.rs` |
| Sidebar workspace list | `issh-app/src/views/sidebar.rs` |
| Terminal pane + ConnectionChip | `issh-app/src/views/terminal_view.rs` |
| Session picker overlay | `issh-app/src/views/session_picker.rs` |
| Tab bar | `issh-app/src/views/tab_bar.rs` |
| Toast 通知 | `issh_ui::toast_*` API |
| Confirm Dialog | `issh-app/src/views/host_form.rs::confirm_delete` |
...
4. 加 primitive 时：
   - 在 `issh-ui/src/components/<name>.rs` 实现
...
- `crates/issh-ui/src/theme/{tokens,typography,anatomy,motion}.rs` —— token 实现源码（每个文件顶部 doc comment 是权威）
