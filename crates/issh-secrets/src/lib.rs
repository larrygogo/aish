//! issh-secrets — 跨平台 keyring 凭证存储。
//!
//! service 固定 "aish"，username 用 HostId 的 UUID 字符串。
//! 调用方在 macOS Keychain Access.app / Windows 凭据管理器 搜 "aish" 能列出全部条目。
//!
//! NOTE: service 名保留旧值 "aish"（项目曾叫 aish，2026-05-21 改名 issh）。
//! 改 service 会让老用户已存的密码 / passphrase 立刻读不到（OS keyring 用
//! service+username 双键定位），权衡后选择不改。新装用户也走同一 service，
//! 与品牌名不完全一致但无功能影响。
