...
pub enum SshEvent {
    ...
    /// SFTP 上传成功，path 是远端绝对路径（如 /tmp/issh-clip-123456.png）。
    ImageUploaded {
        conn: ConnectionId,
        path: String,
    },
...
/// 是 tmux -CC 控制模式专用，issh 已回退到 raw attach 路径，attach 后由 tmux
...
pub enum TmuxState {
    ...
    /// 用户在 tmux 内 detach 后，issh 不感知（保持 Some），需要重新 list 才会更新。
    Detected {
        sessions: Vec<RemoteSession>,
...
impl HostFormDraft {
    ...
    pub fn from_config(cfg: &HostConfig) -> Self {
        let (auth_kind, key_path) = match &cfg.auth {
            issh_types::SshAuth::KeyFile { path, .. } => {
                (AuthKind::KeyFile, path.display().to_string())
            }
            issh_types::SshAuth::Password { .. } => (AuthKind::Password, String::new()),
            issh_types::SshAuth::Agent => (AuthKind::KeyFile, String::new()),
        };
        Self {
...
mod tests {
    ...
    fn draft_keyfile_into_config_validates_key_file_exists() {
        let draft = HostFormDraft {
            ...
            user: "root".into(),
            auth_kind: AuthKind::KeyFile,
            key_path: "/nonexistent/path/issh_test_only".into(),
            password: "".into(),
            error: None,
        };
        assert!(draft
    ...
    fn draft_keyfile_into_config_succeeds_with_existing_key() {
        ...
        assert!(matches!(cfg.auth, issh_types::SshAuth::KeyFile { .. }));
    }
    ...
    fn draft_password_new_succeeds_with_password() {
        ...
        let cfg = draft.into_config(None).unwrap();
        match cfg.auth {
            issh_types::SshAuth::Password { password } => assert_eq!(password, "secret"),
    ...
    fn draft_password_edit_allows_empty_password() {
        ...
        let cfg = draft.into_config(Some(id)).unwrap();
        match cfg.auth {
            issh_types::SshAuth::Password { password } => assert_eq!(password, ""),
            _ => panic!("expected Password variant"),
        }
    }
    ...
    fn draft_from_config_password_keeps_password_empty() {
        let host = HostConfig {
            ...
            port: 22,
            user: "root".into(),
            auth: issh_types::SshAuth::Password {
    ...
    fn image_uploaded_event_carries_path() {
        ...
        let event = SshEvent::ImageUploaded {
            conn,
            path: "/tmp/issh-clip-123.png".into(),
        };
        match event {
            SshEvent::ImageUploaded { conn: c, path } => {
                assert_eq!(c, conn);
                assert_eq!(path, "/tmp/issh-clip-123.png");
            }
            _ => panic!("wrong variant"),
        }
    }
