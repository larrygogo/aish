...
impl gpui::AssetSource for AppAssets {
    fn load(&self, path: &str) -> gpui::Result<Option<std::borrow::Cow<'static, [u8]>>> {
        // 先 forward 到 issh-ui（icons/x.svg / icons/distros/ubuntu.svg 等）
        if let Some(bytes) = gpui::AssetSource::load(&issh_ui::IsshUiAssets, path)? {
            return Ok(Some(bytes));
        }
        // issh-app 本地资源：
        // - logo.svg 单色 path（如果未来需要 svg() monochrome 渲染）
        // - issh-icon.png 真品牌多色 logo（titlebar img() 用，保留蓝/黑双色）
        match path {
            "logo.svg" => Ok(Some(std::borrow::Cow::Borrowed(include_bytes!(
                "../assets/logo.svg"
            )))),
            "issh-icon.png" => Ok(Some(std::borrow::Cow::Borrowed(include_bytes!(
                "../../../assets/icons/issh-128.png"
            )))),
            _ => Ok(None),
        }
    }
...
pub fn run() {
    ...
    application()
        ...
        .run(move |cx: &mut App| {
            // 注册 issh-ui Theme global（必须在创建任何 view / 调用 theme(cx) 之前）
            ...
            let toast_manager = cx.new(issh_ui::ToastManager::new);
                    ...
                    state_for_loop.update(cx, |state, cx| match event {
                        ...
                        SshEvent::Disconnected { conn, reason } => {
                            ...
                            if let DisconnectReason::NetworkError(ref e) = reason {
                                ...
                                issh_ui::toast_error(cx, format!("{}: 连接中断 — {}", label, e));
                            }
                        ...
                        SshEvent::Error { conn, kind, msg } => {
                            ...
                            issh_ui::toast_error(cx, user_msg);
                        ...
                        SshEvent::TmuxMouseDisabled { conn } => {
                            ...
                            // click/drag/wheel 在 tmux 内全失效（issh 的 SGR
                        ...
                        SshEvent::ImageUploadFailed { conn, msg } => {
                            ...
                            issh_ui::toast_error(cx, format!("{}: 图片上传失败 — {}", label, msg));
                        }
            ...
            // Mica，让 issh 看着像系统原生应用。Linux 多数 WM 不支持 vibrancy →
            // 回退 Opaque
            ...
            let window_options = WindowOptions {
                ...
                titlebar: Some(TitlebarOptions {
                    title: Some(SharedString::from("issh")),
                    // 隐掉系统原生 titlebar 内容（macOS/Windows），由 RootView
                    // 顶部自绘 36px strip：issh 标识 + 拖拽区 + 三按钮。
                    // 注意：启用后必须保证自绘按钮逻辑正确（remove_window 等），
                    // 否则用户无法关窗口。
                    appears_transparent: true,
                    ..Default::default()
                }),
                app_id: Some("issh".to_string()), // Linux WM_CLASS，与 .desktop 匹配
                ..Default::default()
            };
...
struct RootView {
    ...
    toast_manager: Entity<issh_ui::ToastManager>,
...
impl Render for RootView {
    fn render(&mut self, _window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        let main_body: gpui::AnyElement = match sidebar {
            ...
            SidebarTab::Terminal => {
                if tabs_empty {
                    ...
                    // M35 T11 移除：ConnectionBar 与 tab_bar tab title 信息
                    // 重复（都显示 host label），违反 earn-every-pixel 原则。
                    // tab item 内 phase status dot + host label + SSH badge 已
                    // 充分覆盖识别需求，删除 ConnectionBar 节省 24px 终端高度。
                    // 完整 user@host:port / tmux session 详情如未来需要可走
                    // tab item hover tooltip 路径，不在 always-on UI 占位。
                    let mut col = div()
                        .size_full()
                        .flex()
                        .flex_col()
        ...
        //   布局：[80px 系统占位][拖拽区 + 居中 logo+issh]。
        // - Windows/Linux：系统不提供任何窗口控件，全部自绘。
        //   布局：[左 logo+issh][中拖拽][右 min/max/close]。
        //
        // logo + 标题：issh 真品牌 logo + "issh" 文字。
        // 注意：GPUI 的 svg() 元素是 **monochrome 模式** —— 整个 SVG 用 text_color
        // 染色覆盖原 fill，无法显示多色 logo。issh 品牌 logo 是蓝+黑双色 + 装饰
        // SSH 钥匙，必须用 img() 加载 PNG 才保留原色。
        // assets/icons/issh-128.png 是项目品牌图标的 128×128 PNG（多色 raster），
        // 通过 AppAssets 的 path="issh-icon.png" 暴露给 GPUI img() Embedded
        ...
        // - macOS 上把 logo+issh 文字塞进拖拽区并居中，避开左上红绿灯
            ...
            .when(is_macos, |d| {
                d.justify_center().gap(px(8.0)).child(
                    div()
                        ...
                        .items_center()
                        .gap(px(8.0))
                        .child(gpui::img("issh-icon.png").w(px(22.0)).h(px(22.0)))
                        .child(
                            div()
                                .text_size(px(13.0))
