...
use issh_types::TabId;

use issh_ui::{ContextMenu, DropdownMenu, IconName, MenuItem, TextInput};
...
impl gpui::Render for TabDragPreview {
    fn render(&mut self, _w: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        div()
            ...
            .shadow(issh_ui::elevation_2(t.kind))
...
pub struct TabBarView {
    ...
    /// 用 issh_ui::TextInput 替代之前手糊 div + on_key_down，自动获得 IME /
    ...
    close_buttons: std::collections::HashMap<TabId, Entity<issh_ui::IconButton>>,
    /// M34: 每个 tab 的 TabItem entity（hover transition + press feedback），
    /// 按 TabId 索引。同 close_buttons retain + ensure 模式。
    tab_items: std::collections::HashMap<TabId, Entity<issh_ui::TabItem>>,
}
...
impl TabBarView {
    ...
    /// 处理 tab 标题点击。
    /// - click_count >= 2 → 进 inline 编辑模式（用 issh_ui::TextInput entity）
...
impl Render for TabBarView {
    fn render(&mut self, window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        if let Some(tab_id) = self.menu_tab_id {
            ...
            let menu = DropdownMenu::new("tab-context-menu")
                .items(vec![
                    ...
                    MenuItem::new("折叠到主页"),
                    MenuItem::new("关闭").icon(IconName::X).shortcut(
        ...
        for tab_id in &tab_ids_snapshot {
            if !self.close_buttons.contains_key(tab_id) {
                ...
                let btn = cx.new(move |cx| {
                    let mut b = issh_ui::IconButton::new(
        ...
        for tab_id in &tab_ids_snapshot {
            if !self.tab_items.contains_key(tab_id) {
                ...
                let ti = cx.new(move |cx| {
                    let mut t =
                        issh_ui::TabItem::new(gpui::SharedString::from(format!("tab-{}", id)), cx);
            ...
            app.tabs
                ...
                .map(|t| {
                    ...
                    if is_editing {
                        ...
                        .h(px(36.0))
                        .my(px(2.0))
                        // M39: editing tab 宽度跟 TabItem 一致 (min 140 / max
                        // 200, 用户反馈「即使在编辑状态下也要保持」)。input
                        // 内部 overflow_hidden 裁超长 title 防溢出邻 tab。
                        .min_w(px(140.0))
                        .max_w(px(200.0))
                        .flex_shrink_0()
                        ...
                        // 末尾可能看不见（issh_ui::TextInput 暂未实现水平滚动
                    ...
                    let prefix: gpui::AnyElement = if is_connection {
                        ...
                        match avatar {
                            Some(crate::avatar::OsAvatar::Svg { icon, bg }) => div()
                                ...
                                .bg(gpui::rgb(bg))
                                .rounded_full()
                                .child(issh_ui::icon(icon).size(px(10.0)).text_color(gpui::white()))
                                .into_any_element(),
                    ...
                    // — 整个 issh 当前只有 SSH 连接，徽标冗余。保留 close_btn。
                    let suffix = div()
                        .flex()
                        .flex_row()
                        .items_center()
                ...
                TabRenderData::Normal {
                    ...
                    div()
                        ...
                        .drag_over::<DraggedTab>(|style, _dragged, _w, cx| {
                            let mut s = style;
                            s.background = Some(issh_ui::theme(cx).colors.accent_active.into());
                            s
                        })
                        .on_mouse_down(
        ...
        let plus_btn = div()
            ...
            .child(
                issh_ui::icon(issh_ui::IconName::Plus)
                    .size(px(16.0))
        ...
        let arrow_left = div()
            ...
            .child(
                ...
                // → svg 不画（之前箭头不显示的根因）。
                issh_ui::icon(issh_ui::IconName::ChevronLeft)
                    .size(px(14.0))
                    .text_color(colors.muted_foreground),
            );
        ...
        let arrow_right = div()
            ...
            .child(
                issh_ui::icon(issh_ui::IconName::ChevronRight)
                    .size(px(14.0))
                    .text_color(colors.muted_foreground),
            );
