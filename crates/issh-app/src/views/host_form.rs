//! 添加 / 编辑 / 删除确认 modal。
//!
//! M12 重写为基于 `issh_ui::Dialog + Tabs + TextInput` 的组件化版本。
//!
...
use issh_types::HostId;
use issh_ui::{theme, Button, Dialog, TextInput, TypographyExt};
        ...
        let body: gpui::AnyElement = div()
            ...
            .child(
                div()
                    ...
                    .gap(spacing.px_3)
                    .child(
                        issh_ui::Radio::new("host-form-auth-keyfile")
                            .label("私钥文件")
                    ...
                    .child(
                        issh_ui::Radio::new("host-form-auth-password")
                            .label("密码")
...
fn field_row(
    ...
    div()
        ...
        .child(input)
        .when_some(error, |d, msg| d.child(issh_ui::InlineError::new(msg)))
}
