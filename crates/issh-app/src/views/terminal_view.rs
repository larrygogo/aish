...
pub struct TerminalView {
    ...
    last_pty_size: Option<(issh_types::ConnectionId, u16, u16)>,
...
impl TerminalView {
    pub fn new(
        ...
        let reconnect_btn = cx.new(|cx| {
            let mut b = issh_ui::Button::new("terminal-reconnect", cx);
        ...
        let edit_host_btn = cx.new(|cx| {
            let mut b = issh_ui::Button::new("terminal-edit-host", cx);
            b.label("编辑主机")
                .secondary()
        ...
        let copy_error_btn = cx.new(|cx| {
            let mut b = issh_ui::Button::new("terminal-copy-error", cx);
            b.label("复制错误")
                .secondary()
                .on_click(move |_ev, _w, cx| {
    ...
    fn paste(&mut self, conn: issh_types::ConnectionId, cx: &mut Context<Self>) {
    ...
    fn handle_copy_error(&mut self, cx: &mut Context<Self>) {
        ...
        issh_ui::toast_success(cx, "错误信息已复制到剪贴板");
    }
...
impl Render for TerminalView {
    fn render(&mut self, window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        let phase_overlay: Option<gpui::AnyElement> = conn.and_then(|c| {
            ...
            let theme_ref = issh_ui::theme(cx);
            ...
            match phase {
                ...
                Some(ConnectionPhase::Disconnected { reason }) => {
                    ...
                    let error_state = issh_ui::ErrorState::new("terminal-disconnected")
                        .icon(issh_ui::IconName::AlertCircle)
                        .title(title)
                        .description(reason)
                        .action(actions);
                    Some(
...
fn take_snapshot(
    conn: Option<issh_types::ConnectionId>,
    state: &Entity<AppState>,
    cx: &mut App,
) -> Option<GridSnapshot> {
    let conn = conn?;
...
mod tests {
    ...
    fn mk_state_with_host() -> (AppState, HostId) {
        let cfg = issh_types::HostConfig {
            id: HostId::new(),
            label: "v".into(),
            host: "1.2.3.4".into(),
            port: 22,
            user: "root".into(),
            auth: issh_types::SshAuth::KeyFile {
                path: std::path::PathBuf::from("/tmp/k"),
                passphrase: String::new(),
            },
            env_profile: None,
            capabilities: issh_types::HostCapabilities::default(),
        };
        let id = cfg.id;
