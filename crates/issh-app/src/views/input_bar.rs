...
//!
//! 文字输入部分使用 issh_ui::TextInput（含 cursor blink / selection / IME / 复制粘贴）。
...
impl InputBarView {
    pub fn new(
        ...
        let pick_btn = cx.new(|cx| {
            let mut b = issh_ui::IconButton::new("input-bar-pick", issh_ui::IconName::Plus, cx);
        ...
        let send_btn = cx.new(|cx| {
            let mut b = issh_ui::Button::new("input-bar-send", cx);
    ...
    fn add_image_from_rgba(
        ...
        let ts = std::time::SystemTime::now()
            ...
            .unwrap_or_default()
            .as_millis();
        let path = std::env::temp_dir().join(format!("issh-paste-{}.png", ts));
