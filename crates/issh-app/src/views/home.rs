...
use issh_types::{ConnectionId, HostId};
use issh_ui::{Button, CardEntity, IconButton, TypographyExt};
...
pub struct HomeView {
    ...
    header_add_btn: Entity<issh_ui::Button>,
    /// M31：空 hosts 状态 EmptyState 内的 add button（条件显示）。
    empty_add_btn: Entity<issh_ui::Button>,
    /// M31：hosts.json 加载失败时 ErrorState 内的 retry button（条件显示）。
    retry_btn: Entity<issh_ui::Button>,
...
impl HomeView {
    pub fn new(
        ...
        let context_menu = cx.new(issh_ui::ContextMenu::new);
        ...
        let header_add_btn = cx.new(|cx| {
            let mut b = issh_ui::Button::new("home-add-host-btn", cx);
            b.label("+ 添加主机")
                .primary()
        ...
        let empty_add_btn = cx.new(|cx| {
            let mut b = issh_ui::Button::new("home-empty-add-host", cx);
            b.label("+ 添加主机")
                .primary()
                .on_click(move |_ev, _w, cx| {
        ...
        let retry_btn = cx.new(|cx| {
            let mut b = issh_ui::Button::new("home-hosts-load-retry", cx);
    ...
    fn copy_ssh_command(&self, host_id: HostId, cx: &mut Context<Self>) {
        ...
        if let Some(cfg) = cfg {
            ...
            issh_ui::toast_success(cx, format!("已复制：{}", cmd));
        }
    }
    ...
    fn handle_retry_load_hosts(&mut self, cx: &mut Context<Self>) {
        match crate::persistence::load_hosts() {
            Ok(hosts) => {
                ...
                issh_ui::toast_success(cx, "主机列表已恢复");
            }
            Err(e) => {
                let msg = format!("{}", e);
                self.state.update(cx, |s, cx| {
                ...
                issh_ui::toast_error(cx, format!("加载失败：{}", msg));
            }
        }
    }
    ...
    fn handle_card_click(&mut self, host_id: HostId, cx: &mut Context<Self>) {
        let (conn_id, config, label) = self.state.update(cx, |s, cx| {
            ...
            let tab_id = issh_types::TabId::new();
            let tab = Tab {
                id: tab_id,
...
impl Render for HomeView {
    fn render(&mut self, _window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        if let Some(host_id) = self.menu_host_id {
            ...
            let menu = issh_ui::DropdownMenu::new("host-context-menu")
                .items(vec![
                    issh_ui::MenuItem::new("编辑").icon(issh_ui::IconName::Pencil),
                    issh_ui::MenuItem::new("复制 SSH 命令"),
                    issh_ui::MenuItem::new("删除").icon(issh_ui::IconName::Trash),
                ])
        ...
        for conn_id in &conns_snapshot {
            ...
            if !self.active_cards.contains_key(conn_id) {
                ...
                let weak = cx.weak_entity();
                let primary = issh_ui::theme(cx).colors.primary;
        ...
        let (
            ...
            let header = div()
                ...
                .child(
                    ...
                    div()
                        .typography(issh_ui::TypeRole::Title1, theme)
                        .child("主页"),
                )
                .child(add_btn);
                ...
                for (conn_id, snap) in sorted_previews.iter().copied() {
                    ...
                    let mut header_row = div()
                        ...
                        .child(
                            div()
                                ...
                                .whitespace_nowrap()
                                .text_ellipsis()
                                .typography(issh_ui::TypeRole::Title3, theme)
                        ...
                        header_row = header_row
                            ...
                            .child(
                                ...
                                div()
                                    ...
                                    .items_center()
                                    .gap_1()
                                    .typography(issh_ui::TypeRole::Code, theme)
                                    .text_color(colors.muted_foreground)
                                    .child(
                                        issh_ui::icon(issh_ui::IconName::Tmux)
                                            .size(px(12.0))
                                            .text_color(colors.muted_foreground),
                                    )
                                    .child(format!("tmux:{}", t)),
                    ...
                    let meta_row = div()
                        ...
                        .child(
                            div()
                                .flex_shrink_0()
                                .typography(issh_ui::TypeRole::Caption, theme)
                                .text_color(colors.muted_foreground)
                                .child(duration_str),
                        );
                    ...
                    let preview_layer: gpui::AnyElement = match branch {
                        ...
                        PreviewBranch::Loading => div()
                            ...
                            .gap_2()
                            .child(
                                issh_ui::icon(issh_ui::IconName::Loader)
                                    .size(px(16.0))
                                    .text_color(colors.muted_foreground),
                            )
                            .child(
                        ...
                        PreviewBranch::DisconnectedHint => div()
                            ...
                            .child(
                                issh_ui::icon(issh_ui::IconName::AlertTriangle)
                                    .size(px(16.0))
                    ...
                    let inner = div()
                        ...
                        //   下不一定严格生效，min/max 兜底
                        // - aspect 1.6 → 1.3 (用户反馈太宽)，更接近 4:3
                        // - 宽 [200, 260]: max 320 → 260
            ...
            // 「继续工作」是 verb-driven 引导（用户打开 issh 大概率为恢复
                ...
                .map(|h| {
                    ...
                    let avatar: gpui::AnyElement = match os_avatar {
                        Some(crate::avatar::OsAvatar::Svg { icon, bg }) => div()
                            ...
                            .bg(rgb(bg))
                            .rounded_xl()
                            .child(issh_ui::icon(icon).size(px(22.0)).text_color(gpui::white()))
                            .into_any_element(),
                    ...
                    let time_meta = div()
                        ...
                        .items_center()
                        .gap_1()
                        .child(div().typography(issh_ui::TypeRole::Caption, theme).child(
                            match last_conn_str {
                                Some(s) => s,
                                None => "未连接".to_string(),
                            },
        ...
        let empty_hint: Option<gpui::AnyElement> = if let Some(err) = load_error {
            Some(
                issh_ui::ErrorState::new("home-hosts-load-failed")
                    .icon(issh_ui::IconName::FileQuestion)
                    .title("加载主机列表失败")
                    .description(err)
            ...
            Some(
                issh_ui::EmptyState::new("home-no-hosts")
        ...
        div()
            ...
            .bg(bg_color)
            .child(
                issh_ui::ScrollPage::new("home-scroll")
...
impl HomeView {
    ...
    pub fn context_menu_entity(&self) -> Entity<issh_ui::ContextMenu> {
