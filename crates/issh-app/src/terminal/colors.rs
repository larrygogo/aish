...
/// 让底下字符仍可见。
pub fn selection_bg_for(kind: issh_ui::ThemeKind) -> gpui::Hsla {
    use gpui::hsla;
...
pub fn to_gpui(color: AlacColor, is_fg: bool, kind: issh_ui::ThemeKind) -> Hsla {
    match color {
...
fn named_to_gpui(named: NamedColor, is_fg: bool, kind: issh_ui::ThemeKind) -> Hsla {
...
fn indexed_to_gpui(idx: u8, is_fg: bool, kind: issh_ui::ThemeKind) -> Hsla {
    if idx < 16 {
...
mod tests {
    ...
    const DARK: issh_ui::ThemeKind = issh_ui::ThemeKind::Dark;
    const LIGHT: issh_ui::ThemeKind = issh_ui::ThemeKind::Light;
