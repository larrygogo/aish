...
use issh_types::{ConnectionId, HostConfig, RemoteSession};
use tokio::sync::mpsc;
...
pub(crate) async fn connection_task(
    ...
    use issh_secrets::{SecretError, SecretStore};
    use issh_ssh::{ChannelMsg, SshClient};
    use issh_types::SshAuth;
    ...
    let session = match SshClient::connect(&effective_config).await {
        ...
        Err(err) => {
            let kind = match err.kind() {
                issh_ssh::SshErrorKind::ConnectFailed => SshErrorKind::ConnectFailed,
                issh_ssh::SshErrorKind::AuthFailed => SshErrorKind::AuthFailed,
                issh_ssh::SshErrorKind::Io => SshErrorKind::Io,
                issh_ssh::SshErrorKind::Protocol => SshErrorKind::Protocol,
            };
            let _ = event_tx
                .send(SshEvent::Error {
                    conn,
    ...
    let mut attached_session: Option<issh_types::SessionId> = None;
    loop {
        tokio::select! {
            ...
            cmd = cmd_rx.recv() => match cmd {
                ...
                Some(SessionCommand::AttachTmux { session: sess_id }) => {
                    ...
                    // 内 prefix+d detach 后自然回到 shell 提示符。issh 不再用 -CC
                ...
                Some(SessionCommand::UploadImage { data }) => {
                    ...
                    tokio::spawn(async move {
                        let ts = std::time::SystemTime::now()
                            ...
                            .unwrap_or_default()
                            .as_millis();
                        let remote_path = format!("/tmp/issh-clip-{}.png", ts);
                ...
                Some(SessionCommand::UploadBatch { images, text }) => {
                    ...
                    tokio::spawn(async move {
                        ...
                        for (i, (bytes, ext)) in images.iter().enumerate() {
                            let remote_path = format!("/tmp/issh-clip-{}-{}.{}", ts, i, ext);
                            let upload =
...
pub fn encode_key(key: &str, ctrl: bool, alt: bool) -> Vec<u8> {
    if ctrl {
        // M37: Ctrl + 特殊键的兼容编码
        // - Ctrl+Enter → LF (0x0A) — readline / Claude Code 等多数 TUI 把 LF 当
        //   换行而不是 submit（普通 Enter = CR 0x0D 触发 submit）。比 CSI u 协议
        //   兼容性更好（CSI u 需 app 主动 enable kitty keyboard protocol）
        // - Ctrl+Tab / Ctrl+Backspace 走 CSI u 协议（kitty / modifyOtherKeys），
        //   modern TUI 大概率能识别
    ...
    s.lines()
        .filter_map(|line| {
            ...
            let activity: i64 = parts
                ...
                .unwrap_or(0);
            Some(RemoteSession {
                id: issh_types::SessionId::new(id_trimmed),
...
async fn os_detect_task(
    host_id: issh_types::HostId,
    client: issh_ssh::SshClient,
    event_tx: mpsc::Sender<SshEvent>,
) {
...
mod tests {
    ...
    async fn password_empty_no_keyring_entry_emits_auth_error() {
        ...
        use issh_types::{ConnectionId, HostCapabilities, HostConfig, SshAuth};

        let cfg = HostConfig {
            id: issh_types::HostId::new(),
            label: "no-pwd".into(),
            host: "127.0.0.1".into(),
            port: 22,
            user: "root".into(),
    ...
    /// M37: Ctrl + 特殊键的兼容编码
    /// - Ctrl+Enter → LF (0x0A) 让 readline/CC 当换行而非 submit
    /// - Ctrl+Tab / Ctrl+Backspace → CSI u 协议
    #[test]
    fn encode_ctrl_special_keys() {
        // Ctrl+Enter → LF (0x0A) — CR (0x0D) 是 submit，LF 是 newline
        assert_eq!(encode_key("enter", true, false), vec![b'\n']);
        // Ctrl+Tab → CSI u "9;5u" (9 = Tab keycode, 5 = Ctrl modifier)
        assert_eq!(encode_key("tab", true, false), b"\x1b[9;5u".to_vec());
        // Ctrl+Backspace → CSI u "127;5u" (127 = Backspace keycode)
        assert_eq!(
