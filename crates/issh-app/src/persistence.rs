//! hosts.json 跨平台持久化读写。
//!
//! 路径：{config_dir}/issh/hosts.json
//!   - Windows: %APPDATA%\issh\hosts.json
//!   - macOS:   ~/Library/Application Support/issh/hosts.json
//!   - Linux:   ~/.config/issh/hosts.json
//!
...
use issh_types::{HostConfig, HostId, SshAuth};

/// 配置子目录名。
const APP_DIR_NAME: &str = "issh";
/// hosts 文件名。
...
mod tests {
    use super::*;
    use issh_types::{HostCapabilities, HostId, SshAuth};
    ...
    fn save_then_load_roundtrip() {
        let dir = tempdir().unwrap();
        let path = dir.path().join("issh").join("hosts.json");
    ...
    fn hosts_json_path_returns_some_on_typical_env() {
        ...
        assert!(
            s.ends_with("issh/hosts.json") || s.ends_with("issh\\hosts.json"),
            "unexpected path: {}",
            s
        );
    }
    ...
    fn save_password_host_does_not_panic() {
        // issh-app 测试时 SecretStore 调真 keyring（生产路径），
        // 我们只验证 save_hosts_to 不 panic 且 hosts.json 正确写入。
        // keyring 实际值由 issh-secrets 自身的 mock 测试 + Task 8 端到端手测覆盖。
        let dir = tempdir().unwrap();
        let path = dir.path().join("hosts.json");
